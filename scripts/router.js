 ...
 */
router.setState = function(state, pushBool) {
  /**
  ...
  //only push our state if we dont use the back button
    if(state.pageState == "Home") {
      ...
      else if(state.pageState == "Entry"){
        if(pushBool == false){
          history.pushState(state,"", '#' + (state.pageState + state.entryCount));
        }
        
        body.className = 'single-entry';
        h1.innerHTML = 'Entry ' + state.entryCount;
        let addEntry = document.createElement('entry-page');
        addEntry.entry = state.entry;
        let entryPage = document.getElementsByTagName("entry-page");
        
        body.appendChild(addEntry);
        body.removeChild(entryPage[0])
      }
      else if(state.pageState == "Settings"){
        if(pushBool == false){
          history.pushState(state, "", "#" + (state.pageState));
        }
        
        body.className = 'settings';
        h1.innerHTML = state.pageState;
      }
}